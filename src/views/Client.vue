...
.data-list-article {display: flex; align-items: center; width: 40%; position: relative;}
...
<template>
  <div class="page">
    ...
    <!-- 广告图片 <div class="data-ad mt-lg">
      <img src="../assets/images/data-banner.png">
    </div> -->
    <div class="data-list temporary-class">
      <van-pull-refresh v-model="loading" @refresh="refreshList"> 
        ...
        >
          ...
          >
            ...
            <span class="data-list-op" :class="{'text-info': item.operate_type != 2}">{{ item.operate_type == 2 ? '查看' : (item.operate_type == 1 ? '转发' : '点击') }}</span>
            <span>了你的{{ item.operate_type == 3 ? '广告' : '新闻' }}</span>
